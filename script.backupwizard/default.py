...
ADDON        =  xbmcaddon.Addon(id='script.backupwizard')
...
if zip=='' and ADDON.getSetting('email')=='':
    if dialog.yesno("BackUp Wizard", "You Have Not Set Your Storage Path", 'Set The Storage Path Now ?',''):
        ADDON.openSettings()
...
def RepoInstaller():
    ...
    if not os.path.exists(path):
        f = open(path, mode='w')
        f.write('<sources><files><source><name>Repo[COLOR gold] [B]L[/B] [/COLOR][COLOR orange]ON[/COLOR]</name><path pathversion="1">http://tuNube/CloudBackup</path></source></files></sources>')
        f.close()
        return
    ...
    if not'xunitytalk.me/xfinity' in str:
        if '</files>' in str:
            str = str.replace('</files>','<source><name>Repo[COLOR gold] [B]L[/B] [/COLOR][COLOR orange]ON[/COLOR]</name><path pathversion="1">http://tuNube/CloudBackup</path></source></files>')
            f = open(path, mode='w')
            f.write(str)
            f.close()
        else:
            str = str.replace('</sources>','<files><source><name>Repo[COLOR gold] [B]L[/B] [/COLOR][COLOR orange]ON[/COLOR]</name><path pathversion="1">http://tuNube/CloudBackup</path></source></files></sources>')
            f = open(path, mode='w')
            f.write(str)
            f.close()
...
def BACKUP():  
    if zip == '':
        dialog.ok('BackUp Wizard','You have not set your ZIP Folder or Email\nPlease update the addon settings and try again.','','')
    ...
    dp.create("BackUp Wizard","Backing Up",'', 'Please Wait')
    ...
    for base, dirs, files in os.walk(to_backup):
        for file in files:
            ...
            if not 'temp' in dirs:
                if not 'script.backupwizard' in dirs:
                   import time
                   CUNT= '01/01/1980'
    ...
    zipobj.close()
    dp.close()
    dialog.ok("BackUp Wizard", "You Are Now Backed Up", '','')
...
def RESTORE():
    ...
    dialog = xbmcgui.Dialog()
    if zip == '' and ADDON.getSetting('email')=='':
        dialog.ok('BackUp Wizard','You have not set your ZIP Folder.\nPlease update the addon settings and try again.','','')
        ADDON.openSettings(sys.argv[0])
        
    lib=xbmc.translatePath(os.path.join(zip,'backup.zip'))
    READ_ZIP(lib)
    dp.create("BackUp Wizard","Checking ",'', 'Please Wait')
    ...
    Kodi17()
    dialog.ok("BackUp Wizard", "PLEASE REBOOT YOUR BOX IF HOMESCREEN HAS NOT CHANGED", "","")
...
def RESTORE_ZIP_FILE(name,url):
    ...
    if 'Backup' in name:
        ...
        xbmc.executebuiltin("UpdateAddonRepos")
        Kodi17()
        dialog.ok("BackUp Wizard", "You Are Now Restored", '','')
...
def RESTORE_BACKUP_XML(name,url,description):
    if 'Backup' in name:
        ...
        if 'guisettings.xml' in description:
            ...
            f.write(TO_READ)
            f.close()  
    dialog.ok("BackUp Wizard", "", 'All Done !','')
...
def EmailPass():
    if ADDON.getSetting('email_pass')=='':
        ...
        if dialog.yesno("BackUp Wizard", "Do you Want To Save Your Password ?", 'Its Fully AES Encrypted','Just Enter Secret 4 Digit Number'):
...
def ResetPass():   
    ...
    dialog.ok('BackUp Wizard','Pass Reset','','')
...
def send_email(TOWHO,LOG):
    ...
    import zipfile
    dp = xbmcgui.DialogProgress()
    dp.create("BackUp Wizard",'Logging Into Your Email')
    dp.update(0)
...
def DownloadAttachment(THEFILE):
    ...
    dp.create("BackUp Wizard","Login To Email ",'', 'Please Wait')
    ...
    try:
        ...
        if typ != 'OK':
            return dialog.ok('BackUp Wizard','Not able to sign in!','','')
        ...
        if typ != 'OK':
            return dialog.ok('BackUp Wizard','Error searching Inbox.','','')
        ...
        for msgId in data[0].split():
            ...
            if typ != 'OK':
                return dialog.ok('BackUp Wizard','Error fetching mail.','','')
...
def RESTORE_EMAIL_FILE(ZIPFILE):
    ...
    xbmc.executebuiltin("UpdateAddonRepos")
    Kodi17()
    dialog.ok("BackUp Wizard", "You Are Now Restored", 'If New Updates Dont Come Through Reboot Kodi','A Couple Times & Manually Update Plugins')
    xbmc.executebuiltin('UpdateLocalAddons ')    
    xbmc.executebuiltin("UpdateAddonRepos")
...
def Show_Dialog(line1,line2,line3):
    dialog = xbmcgui.Dialog()
    dialog.ok('BackUp Wizard', line1,line2,line3)
